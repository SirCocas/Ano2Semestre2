...
#include <stdio.h>
#include "Time.h"
#include "../../../../../../../../../cygwin64/usr/include/assert.h"
...
Time TimeCreate(int hh, int mm, int ss) {
  ...
  Time t = ss+60*mm+3600*hh;  // Fill in the formula for total seconds since midnight
...
int TimeGetMM(Time t) {
    return (t-TimeGetHH(t)*3600)/60;
}

int TimeGetSS(Time t) {
    return (t-TimeGetMM(t)*60-TimeGetHH(t)*3600);
}
